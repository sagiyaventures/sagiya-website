...
export const TelemetryModal: React.FC<TelemetryModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl border border-[#004900]/20 max-w-md w-full p-8 shadow-2xl relative text-center">
        <button onClick={onClose} className="absolute top-5 right-5 p-2 rounded-full hover:bg-stone-100 text-stone-500">
          <X className="w-5 h-5" />
        </button>
        <div className="w-14 h-14 rounded-full bg-[#006400] text-white flex items-center justify-center mx-auto mb-4">
          <Mail className="w-7 h-7" />
        </div>
        <h3 className="font-headline text-xl font-bold text-[#2c160e] mb-2">Interested in a Demo?</h3>
